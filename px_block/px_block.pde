...
void draw(){
  ...
  for (int x = 0; x < img_w; x+=px_block){
    for (int y = 0; y < img_h; y+=px_block){ 
      if (x+px_block > img_w){
        px_block_w = px_block - (x+px_block-img_w);  //reset the width px block to the remainder in the row
      }
      if (y+px_block > img_h){
        px_block_h = px_block - (y+px_block-img_h); //reset the width px block to the remainder in the column     
      }        
      loc = x+y*img_w;
